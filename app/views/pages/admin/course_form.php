<div class="container mx-auto px-4 py-8">
    ...
    <div class="bg-white p-6 rounded-lg shadow-xl max-w-2xl mx-auto">
        <form action="<?= $action ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                <label for="category" class="block text-gray-700 font-semibold mb-2">Categoria do Curso</label>
                <select name="category" id="category" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                    <option value="" disabled <?= empty($course['category']) ? 'selected' : '' ?>>Selecione uma opção...</option>
                    <option value="0" <?= (isset($course['category']) && $course['category'] === 'Gratuito') ? 'selected' : '' ?>>Gratuito</option>
                    <option value="1" <?= (isset($course['category']) && $course['category'] === 'Pago') ? 'selected' : '' ?>>Pago</option>
                </select>
            </div>
