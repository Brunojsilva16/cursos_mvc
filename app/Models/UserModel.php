...
class UserModel
{
    ...
    {
        // O Connection é obtido via Singleton
    ...
    {
        try {
            $stmt = $this->db->prepare("SELECT * FROM users_app WHERE email = :email LIMIT 1");
            $stmt->bindParam(':email', $email);
            $stmt->execute();

            return $stmt->fetch();
        } catch (PDOException $e) {
            // Trate o erro conforme a necessidade do seu sistema
            return false;
        }
    }
    ...
    {
        try {
            $stmt = $this->db->query("SELECT * FROM users_app ORDER BY id DESC");
